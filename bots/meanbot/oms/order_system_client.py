...
This module keeps meanbot's strategy-facing order_manager shape while sending
...
class OrderSystemClient:
    ...
    def __init__(
        ...
        self.curr_date = (
            curr_date
            or os.getenv("MEANBOT_CURR_DATE")
        ...
        self.month_year = (
            month_year
            or os.getenv("MEANBOT_MONTH_YEAR")
    ...
    def block_bot(self, bot_name: Optional[str] = None, reason: Optional[str] = None) -> Dict[str, Any]:
        """Block new order intake without closing the bot's open trades."""
        resolved_bot_name = str(bot_name or self.bot_name).strip()
        return self._request(
            "POST",
            f"/v1/bots/{resolved_bot_name}/block-orders",
