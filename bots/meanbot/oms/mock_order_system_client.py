...
class MockOrderSystemClient(OrderSystemClient):
    ...
    def block_bot(self, bot_name: Optional[str] = None, reason: Optional[str] = None) -> Dict[str, Any]:
        self._mock_order_intake_blocked = True
        self._mock_order_intake_reason = str(reason or "").strip()
        return self._mock_kill_switch_response(bot_name)

    def resume_bot(self, bot_name: Optional[str] = None, reason: Optional[str] = None) -> Dict[str, Any]:
        self._mock_order_intake_blocked = False
        self._mock_order_intake_reason = str(reason or "").strip()
        return self._mock_kill_switch_response(bot_name)
    ...
    def _mock_kill_switch_response(self, bot_name: Optional[str] = None) -> Dict[str, Any]:
        return {
            "bot_name": str(bot_name or self.bot_name).strip(),
            "kill_enabled": bool(getattr(self, "_mock_order_intake_blocked", False)),
            "reason": str(getattr(self, "_mock_order_intake_reason", "") or ""),
    ...
    def create_trade(
        ...
        if bool(getattr(self, "_mock_order_intake_blocked", False)):
            reason = str(getattr(self, "_mock_order_intake_reason", "") or "order intake blocked")
            raise OrderSystemError(f"bot {self.bot_name} order intake is blocked: {reason}")
    ...
    def _local_account_id(self) -> str:
        ...
        return f"mock-{normalized or 'meanbot'}"
