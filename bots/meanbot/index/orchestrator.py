...
def orchestrator(instruments, strategy, mode=None):
    ...
    logger.info(
        f"Starting meanbot orchestrator for instruments: {instruments}, strategy: {strategy}, mode: {mode}"
    )

    ledger_paths = initialize_local_ledgers_for_modes(list(constants.TRADING_EXECUTION_MODES))
    logger.info(
        f"Initialized local OMS ledgers under {constants.MEANBOT_EXECUTION_RESULTS_DIR} "
        f"for modes: {', '.join(ledger_paths.keys())}"
    )

    param_data = load_param_data(mode) or {}
    if not param_data:
        logger.info("No meanbot params configured yet; continuing with defaults.")

    skip_execution, skip_reason = should_skip_strategy_execution(
        param_data,
        strategy,
        current_date=os.getenv("MEANBOT_CURR_DATE"),
    )
    if skip_execution:
    ...
    if instruments == constants.NIFTY50:
        ...
        logger.info("Meanbot orchestrator setup complete.")
        return True
