...
async def nifty50_engine(strategy, mode, param_data):
    logger.info(f"Starting Meanbot Nifty50 Engine with strategy: {strategy}, mode: {mode}")
    ...
    try:
        ...
        meanrev_vwap_key = str(constants.MEANREV_VWAP).strip().lower()

        strategy_cls = None
        if strategy_key == meanrev_vwap_key:
            from index.nifty50.strategy import MeanRevVwapStrategy
            strategy_cls = MeanRevVwapStrategy
