...
DEFAULT_MEANBOT_FILES_DIR = "files"
MEANBOT_FILES_DIR = (
    _first_env("MEANBOT_FILES_DIR", "BOT_FILES_DIR")
    or DEFAULT_MEANBOT_FILES_DIR
).rstrip("/") or DEFAULT_MEANBOT_FILES_DIR

if tuple(part for part in MEANBOT_FILES_DIR.split("/") if part) in {
    ("bots", "meanbot", "files"),
}:
    MEANBOT_FILES_DIR = DEFAULT_MEANBOT_FILES_DIR

MEANBOT_EXECUTION_RESULTS_DIR = os.path.join(MEANBOT_FILES_DIR, "execution_results")

# Aliases used by the shared OMS client shape copied from other bots.
TRENDOBOT_EXECUTION_RESULTS_DIR = MEANBOT_EXECUTION_RESULTS_DIR
SOLOBOT_EXECUTION_RESULTS_DIR = MEANBOT_EXECUTION_RESULTS_DIR
...
PARAM_PATH = (
    ...
    or os.path.join(MEANBOT_FILES_DIR, "param.yaml")
)

TREND_FILE = os.path.join(MEANBOT_FILES_DIR, "trend.json")
NIFTY50_OPTION_CONTRACTS_FILE = os.path.join(MEANBOT_FILES_DIR, "nifty50_option_contracts.json")
UPSTOX_NSE_INSTRUMENT_FILE = os.path.join(MEANBOT_FILES_DIR, "upstox_nse_instruments.json")
HOLIDAY_LIST_FILE = os.path.join(MEANBOT_FILES_DIR, "holiday_list.json")

NIFTY50 = "nifty50"
NIFTY50_SYMBOL = "NSE_INDEX|Nifty 50"
SUPPORTED_INSTRUMENTS = (NIFTY50,)

MEANREV_VWAP = "meanrev_vwap"
DEFAULT_STRATEGY = MEANREV_VWAP
...
MOCK_FOLDER_PATH = os.path.join(MEANBOT_EXECUTION_RESULTS_DIR, "mock")
...
SANDBOX_FOLDER_PATH = os.path.join(MEANBOT_EXECUTION_RESULTS_DIR, "sandbox")
...
PROD_FOLDER_PATH = os.path.join(MEANBOT_EXECUTION_RESULTS_DIR, "prod")
...
ORDER_EVENT_LOG = os.path.join(MEANBOT_FILES_DIR, "order_event_log.json")
ORDER_LOG = os.path.join(MEANBOT_FILES_DIR, "order_log.csv")
DAILY_PNL = os.path.join(MEANBOT_FILES_DIR, "daily_pnl.csv")
...
def resolve_execution_mode(fallback_mode=None):
    mode = os.getenv("MEANBOT_MODE") or fallback_mode or MOCK
