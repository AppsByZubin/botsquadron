...
DEFAULT_TITANBOT_FILES_DIR = "files"
TITANBOT_FILES_DIR = (
    _first_env("TITANBOT_FILES_DIR", "BOT_FILES_DIR")
    or DEFAULT_TITANBOT_FILES_DIR
).rstrip("/") or DEFAULT_TITANBOT_FILES_DIR

if tuple(part for part in TITANBOT_FILES_DIR.split("/") if part) in {
    ("bots", "titanbot", "files"),
}:
    TITANBOT_FILES_DIR = DEFAULT_TITANBOT_FILES_DIR

TITANBOT_EXECUTION_RESULTS_DIR = os.path.join(TITANBOT_FILES_DIR, "execution_results")

# Aliases used by the shared OMS client shape copied from other bots.
TRENDOBOT_EXECUTION_RESULTS_DIR = TITANBOT_EXECUTION_RESULTS_DIR
SOLOBOT_EXECUTION_RESULTS_DIR = TITANBOT_EXECUTION_RESULTS_DIR
...
PARAM_PATH = (
    ...
    or os.path.join(TITANBOT_FILES_DIR, "param.yaml")
)

TREND_FILE = os.path.join(TITANBOT_FILES_DIR, "trend.json")
NIFTY50_OPTION_CONTRACTS_FILE = os.path.join(TITANBOT_FILES_DIR, "nifty50_option_contracts.json")
UPSTOX_NSE_INSTRUMENT_FILE = os.path.join(TITANBOT_FILES_DIR, "upstox_nse_instruments.json")
HOLIDAY_LIST_FILE = os.path.join(TITANBOT_FILES_DIR, "holiday_list.json")
...
TIMESERIES_TREND_V2 = "timeseries_trend_v2"
DEFAULT_STRATEGY = TIMESERIES_TREND
...
MOCK_FOLDER_PATH = os.path.join(TITANBOT_EXECUTION_RESULTS_DIR, "mock")
...
SANDBOX_FOLDER_PATH = os.path.join(TITANBOT_EXECUTION_RESULTS_DIR, "sandbox")
...
PROD_FOLDER_PATH = os.path.join(TITANBOT_EXECUTION_RESULTS_DIR, "prod")
...
ORDER_EVENT_LOG = os.path.join(TITANBOT_FILES_DIR, "order_event_log.json")
ORDER_LOG = os.path.join(TITANBOT_FILES_DIR, "order_log.csv")
DAILY_PNL = os.path.join(TITANBOT_FILES_DIR, "daily_pnl.csv")
...
def resolve_execution_mode(fallback_mode=None):
    mode = os.getenv("TITANBOT_MODE") or fallback_mode or MOCK
