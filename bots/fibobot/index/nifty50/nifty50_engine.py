...
def _normalize_tick_payload(payload, instrument_keys_set, future_key):
    ...
    oi = payload.get("oi", payload.get("open_interest"))
    if oi is not None:
        try:
            market_ff["oi"] = float(oi)
        except (TypeError, ValueError):
            pass
...
async def nifty50_engine(strategy, mode, param_data):
    logger.info(f"Starting Fibobot Nifty50 Engine with strategy: {strategy}, mode: {mode}")
    ...
    try:
        ...
        fib_ema_atr_vol_key = str(constants.FIB_EMA_ATR_VOL).strip().lower()

        strategy_cls = None
        if strategy_key == fib_ema_atr_vol_key:
            from index.nifty50.strategy import FibEmaAtrVolStrategy
            strategy_cls = FibEmaAtrVolStrategy
        ...
        await _publish_marketfeeder_subscription(nc, instrument_data)
        logger.info(
            f"Published instrument keys to marketfeeder for bot {bot_id}: "
            f"{len(instrument_keys)} instruments: {instrument_keys}"
        )
