...
def orchestrator(instruments, strategy, mode=None):
    ...
    if not param_data:
        logger.info("No fibobot params configured yet; continuing with defaults.")

    skip_execution, skip_reason = should_skip_strategy_execution(
        param_data,
        strategy,
        current_date=os.getenv("FIBOBOT_CURR_DATE"),
    )
    if skip_execution:
    ...
    if instruments == constants.NIFTY50:
        ...
        logger.info("Fibobot orchestrator setup complete.")
        return True
