...
DEFAULT_FIBOBOT_FILES_DIR = "files"
FIBOBOT_FILES_DIR = (
    _first_env("FIBOBOT_FILES_DIR", "BOT_FILES_DIR")
    or DEFAULT_FIBOBOT_FILES_DIR
).rstrip("/") or DEFAULT_FIBOBOT_FILES_DIR

if tuple(part for part in FIBOBOT_FILES_DIR.split("/") if part) in {
    ("bots", "fibobot", "files"),
}:
    FIBOBOT_FILES_DIR = DEFAULT_FIBOBOT_FILES_DIR

FIBOBOT_EXECUTION_RESULTS_DIR = os.path.join(FIBOBOT_FILES_DIR, "execution_results")

# Aliases used by the shared OMS client shape copied from other bots.
TRENDOBOT_EXECUTION_RESULTS_DIR = FIBOBOT_EXECUTION_RESULTS_DIR
SOLOBOT_EXECUTION_RESULTS_DIR = FIBOBOT_EXECUTION_RESULTS_DIR
...
PARAM_PATH = (
    ...
    or os.path.join(FIBOBOT_FILES_DIR, "param.yaml")
)

TREND_FILE = os.path.join(FIBOBOT_FILES_DIR, "trend.json")
NIFTY50_OPTION_CONTRACTS_FILE = os.path.join(FIBOBOT_FILES_DIR, "nifty50_option_contracts.json")
UPSTOX_NSE_INSTRUMENT_FILE = os.path.join(FIBOBOT_FILES_DIR, "upstox_nse_instruments.json")
HOLIDAY_LIST_FILE = os.path.join(FIBOBOT_FILES_DIR, "holiday_list.json")

NIFTY50 = "nifty50"
NIFTY50_SYMBOL = "NSE_INDEX|Nifty 50"
SUPPORTED_INSTRUMENTS = (NIFTY50,)

FIB_EMA_ATR_VOL = "fib_ema_atr_vol"
DEFAULT_STRATEGY = FIB_EMA_ATR_VOL
...
MOCK_FOLDER_PATH = os.path.join(FIBOBOT_EXECUTION_RESULTS_DIR, "mock")
...
SANDBOX_FOLDER_PATH = os.path.join(FIBOBOT_EXECUTION_RESULTS_DIR, "sandbox")
...
PROD_FOLDER_PATH = os.path.join(FIBOBOT_EXECUTION_RESULTS_DIR, "prod")
...
ORDER_EVENT_LOG = os.path.join(FIBOBOT_FILES_DIR, "order_event_log.json")
ORDER_LOG = os.path.join(FIBOBOT_FILES_DIR, "order_log.csv")
DAILY_PNL = os.path.join(FIBOBOT_FILES_DIR, "daily_pnl.csv")
...
def resolve_execution_mode(fallback_mode=None):
    mode = os.getenv("FIBOBOT_MODE") or fallback_mode or MOCK
