...
This module keeps firebot's strategy-facing order_manager shape while sending
...
class OrderSystemClient:
    ...
    def __init__(
        ...
        self.curr_date = (
            curr_date
            or os.getenv("FIREBOT_CURR_DATE")
        ...
        self.month_year = (
            month_year
            or os.getenv("FIREBOT_MONTH_YEAR")
...
def _default_orders_csv(mode: str) -> str:
    ...
    return str(Path(constants.FIREBOT_EXECUTION_RESULTS_DIR) / (mode_key or constants.MOCK) / "order_log.csv")
...
def _default_daily_csv(mode: str) -> str:
    ...
    return str(Path(constants.FIREBOT_EXECUTION_RESULTS_DIR) / (mode_key or constants.MOCK) / "daily_pnl.csv")
...
def _default_events_json(mode: str) -> str:
    ...
    return str(Path(constants.FIREBOT_EXECUTION_RESULTS_DIR) / (mode_key or constants.MOCK) / "order_event_log.json")
